...
export const commitToServer = (dryRun=false, retry=false, opts={}) => (dispatch, getState) => {
  ...
  const {COMMIT_REQUESTED, COMMIT_DRYRUN_REQUESTED, COMMIT_FAILED, COMMIT_SUCCESSFUL, COMMIT_DRYRUN_SUCCESSFUL, COMMIT_SENT} = serverActionTypes;
  if (dryRun) {
    dispatch({type: COMMIT_DRYRUN_REQUESTED});
  } else {
